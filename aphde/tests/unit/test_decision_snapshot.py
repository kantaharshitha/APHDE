...
FIXTURE_CONTEXT_PATH = Path(__file__).resolve().parents[1] / "fixtures" / "decision_weight_loss_snapshot_luteal.json"
...
def test_weight_loss_decision_snapshot_with_luteal_context() -> None:
    strategy = StrategyFactory.create(GoalType.WEIGHT_LOSS)
    signals = SignalBundle(
        trend_slope=0.02,
        volatility_index=0.11,
    ...
    expected = json.loads(FIXTURE_CONTEXT_PATH.read_text(encoding="utf-8-sig"))
    assert actual == expected
